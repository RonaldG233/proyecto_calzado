...
export const catalogoTallaController = () => {
  const headerContainer = document.getElementById("header-container");
  const sidebarContainer=document.getElementById("sidebar-container");
  const contenedor = document.getElementById("contenedorCatalogo");
  const selectTallas = document.getElementById("selectTallas");
  ...
  // Cargar tallas activas en el select
  const cargarTallas = async () => {
    try {
      const res = await fetch(`${URL_TALLAS}/activas`);
      const tallas = await res.json();

      selectTallas.innerHTML = '<option value="">-- Todas las tallas --</option>';
      tallas.forEach(t => {
      ...
      console.error("Error cargando tallas activas:", e);
    }
  };
  ...
  const filtrarPorTalla = () => {
    const codTalla = selectTallas.value;
    if (!codTalla) {
      renderizarProductos(productos);
      return;
    }
    ...
    const filtrados = productos.filter(prod => {
      ...
      return tallasLocal.some(t => t.codTalla == codTalla && t.stock > 0);
    });
  ...
  // CLICK EN LA CARD SOLO SI NO SE HACE CLICK EN BOTÓN, SELECT O INPUT
  ...
  // Eventos
  selectTallas.addEventListener("change", filtrarPorTalla);
